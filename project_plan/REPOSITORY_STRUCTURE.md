...
│   ├── guards/          # IMPLEMENTED (Tasks 4.7-4.9) — input.py: deterministic
│   │                     # input-guardrail boundary (length/scope/advice/PII/injection
│   │                     # checks, rate-limiting strategy documented not implemented);
│   │                     # context.py: context-guardrail boundary (provenance checks
│   │                     # per evidence route, delimited-evidence formatting), wired
│   │                     # into src.generation.minimal; output.py: output-guardrail
│   │                     # boundary (citation validity vs supplied evidence, advice
│   │                     # detection, structured-route numeric provenance re-export),
│   │                     # wired into MinimalGenerator after provider.generate()
│   ├── api/             # IMPLEMENTED (Task 4.10) — schemas.py (typed request/
│   │                     # response models), service.py (handle_query() thin
│   │                     # orchestration facade), dependencies.py (lazy real-
│   │                     # dependency construction), app.py (create_app()
│   │                     # factory: /health, /status, POST /query)
...
| `generation/` | **implemented** | `provider.py` (provider-neutral interface), `openrouter.py` (adapter), `minimal.py`, `citations.py` (Task 1.7), see `project_plan/PHASE1_GENERATION.md`; `ollama_provider.py` — local/offline `GenerationProvider` (zero-cost, `gpt-oss:20b`) and `factory.py` — `get_generation_provider()`, config-driven provider resolution; `openai/gpt-oss-20b` via OpenRouter frozen as the live production model (Task 4.6), see `project_plan/PHASE4_GENERATION_PRODUCTION_INTERFACE.md`; `minimal.py`'s `_format_context()` now delimits each chunk via `src.guards.context.format_evidence_block()` and `MinimalGenerator` runs the Task 4.8 context guard before every provider call (Task 4.8), see `project_plan/PHASE4_CONTEXT_GUARDRAILS.md`; `MinimalGenerator` also runs the Task 4.9 output guard (`src.guards.output.check_dense_output`) immediately after every provider call, before the public `GenerationResult` is built (Task 4.9), see `project_plan/PHASE4_OUTPUT_GUARDRAILS.md` |
...
| `guards/` | **implemented** | `input.py` — deterministic input-guardrail boundary (`check_input()`/`check_input_with_settings()`, `GuardDecision`, `REASON_CODES`): request-length cap, scope enforcement, advice detection, PII regex checks, injection-pattern detection (reuses `src.router.rules.INJECTION_KEYWORDS`/`ADVICE_KEYWORDS`), rate-limiting strategy documented (not implemented — delegated to a future API Gateway) (Task 4.7), see `project_plan/PHASE4_INPUT_GUARDRAILS.md`; `context.py` — context-guardrail boundary (`ContextGuardDecision`, `check_dense_context()`/`check_xbrl_context()`/`check_navigation_context()`, `format_evidence_block()`): provenance-completeness checks per evidence route + explicit BEGIN/END evidence delimiters, wired into `src.generation.minimal._format_context()`/`MinimalGenerator` (Task 4.8), see `project_plan/PHASE4_CONTEXT_GUARDRAILS.md`; `output.py` — output-guardrail boundary (`OutputGuardDecision`, `check_dense_output()`, `check_structured_numeric_output` = `check_xbrl_context` re-exported): citation validity against supplied evidence (reuses `src.eval.citation_integrity.detect_citation_attempts()`), advice detection (reuses `ADVICE_KEYWORDS`), wired into `MinimalGenerator` after `provider.generate()` (Task 4.9), see `project_plan/PHASE4_OUTPUT_GUARDRAILS.md` |
| `api/` | **implemented** | `schemas.py` — `QueryRequest`/`QueryResponse`/`HealthResponse`/`StatusResponse` (Pydantic models only, no pipeline imports); `service.py` — `handle_query()`, the one thin orchestration facade composing Task 4.7's input guard, Task 3.8's router, Task 3.10's structured XBRL lookup, and Task 1.7/4.8/4.9's dense generation pipeline (zero business logic of its own); `dependencies.py` — `build_production_gazetteer()` (real SEC company/CIK data, per Task 3.8's own documented production stand-in) and `build_production_dependencies()` (real BGE/LanceDB/xbrl.duckdb/provider construction, imported lazily only inside the app lifespan); `app.py` — `create_app()` factory, `GET /health`, `GET /status`, `POST /query` (Task 4.10), see `project_plan/PHASE4_FASTAPI_SERVICE.md` |
