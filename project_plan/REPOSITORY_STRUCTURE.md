...
| `generation/` | **implemented** | `provider.py` (provider-neutral interface), `openrouter.py` (adapter), `minimal.py`, `citations.py` (Task 1.7), see `project_plan/PHASE1_GENERATION.md`; `ollama_provider.py` — local/offline `GenerationProvider` (zero-cost, `gpt-oss:20b`) and `factory.py` — `get_generation_provider()`, config-driven provider resolution; `openai/gpt-oss-20b` via OpenRouter frozen as the live production model (Task 4.6), see `project_plan/PHASE4_GENERATION_PRODUCTION_INTERFACE.md` |
