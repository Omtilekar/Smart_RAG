...
| `sql/` | **implemented** | `xbrl_lookup.py` — `XbrlFactIndex`, a `(tag,cik,fiscal_year) -> facts` index built from Task 2.1/2.2's frozen `eligible_facts()` (one query per tag, never per question); `.lookup()` returns `found`/`not_found`/`ambiguous`/`unsupported_tag` (Task 3.10), see `project_plan/PHASE3_XBRL_SQL_PATH.md` |
