"""Task 3.10 - pure logic for structured XBRL SQL path evaluation:
config-hash wrapper, rate aggregation (reusing Task 2.6's
`aggregate_rate` and Task 2.3's `numeric_exact_match` - never a second
implementation of either), and the SQL-path ablation row. No I/O, no
DuckDB, no model, no network, and - like every other Phase 3 pure-logic
module - no dependency on `src.eval.test_access` anywhere in this
module.
"""
...
ABLATION_TABLE_SQL_EXTRA_COLUMNS: tuple[str, ...] = p3.ABLATION_TABLE_SQL_EXTRA_COLUMNS


class Phase3SqlError(ValueError):
    """Raised for a malformed SQL-path evaluation result - never
    silently worked around."""


def compute_sql_config_hash(config: Mapping) -> str:
    """Task 2.10's canonical `semantic_hash()` - never a second,
    independent hashing implementation."""
    return semantic_hash(config)


def sql_rates(*, attempted_flags: Sequence[bool], found_flags: Sequence[bool],
              exact_match_flags: Sequence[bool], trap_leak_flags: Sequence[bool]) -> dict:
    """`attempted_flags` is over every true `xbrl_fact` question (was it
    routed to the SQL path with complete cik/fiscal_year/tag
    extraction?); `found_flags`/`exact_match_flags` are over attempted
    questions only; `trap_leak_flags` is over the unsupported-tag /
    year-outside-window "trap" population (did the SQL path ever return
    `outcome="found"` for a question it must never be able to answer?).
    ...
    return {
        ...
        "sql_found_rate": aggregate_rate(found_flags),
        "sql_exact_match_rate": aggregate_rate(exact_match_flags),
        "trap_leak_rate": aggregate_rate(trap_leak_flags),
    }


def build_sql_ablation_row(
    *, row_id: str, config_hash: str, run_id: str, git_sha: str, eval_scope_sha256: str,
    ...
    xbrl_fact_question_count: int, trap_question_count: int, rates: Mapping, notes: str,
) -> dict:
    return {
        "row_id": row_id, "configuration": "structured_xbrl_sql_path", "status": "phase3_10_xbrl_sql_path",
        ...
        "question_count": xbrl_fact_question_count + trap_question_count,
        ...
        "retrieval": "structured_sql_no_retrieval", "candidate_k": p3.NA,
        ...
        "xbrl_fact_question_count": xbrl_fact_question_count, "trap_question_count": trap_question_count,
        "routing_coverage": rates["routing_coverage"]["value"], "sql_found_rate": rates["sql_found_rate"]["value"],
        "sql_exact_match_rate": rates["sql_exact_match_rate"]["value"], "trap_leak_rate": rates["trap_leak_rate"]["value"],
        "notes": notes,
    }


__all__ = [
    "SQL_TASK", "ABLATION_TABLE_SQL_EXTRA_COLUMNS", "Phase3SqlError",
    "compute_sql_config_hash", "sql_rates", "build_sql_ablation_row",
]
