...
# Standing authorization — never ask for "continue" at normal task boundaries

The user explicitly grants standing authorization to move from one remaining
Phase 3 roadmap task to the next whenever the controller's normal advancement
gates pass.

A successfully completed numbered task is **NOT** a reason to stop and is
**NOT** a user-decision point.

Therefore, while this controller is active:

- NEVER ask "continue?", "shall I proceed?", "ready for the next task?", or
  "type continue" merely because one Phase 3 task finished.
- NEVER end an otherwise healthy controller run just to obtain permission to
  start the next numbered Phase 3 task.
- If the current task is complete, tests pass, provenance is valid, TEST is
  untouched, the selection rule is resolved, and the next roadmap task is
  explicit, immediately begin LOOP STEP 1 for that next task in the same
  controller run.
- A brief progress message between tasks is fine, but it must be followed by
  actual work on the next task rather than a request for confirmation.
- The user's original launch of this controller is the authorization for all
  such normal Phase 3 task transitions.

This standing authorization supersedes any generic sentence in an older
standalone Phase 3 task prompt that says things such as "STOP before the next
task", "do not start the next task automatically", or "wait for the user to
say continue". Those standalone prompts were designed for one-task-at-a-time
interactive use; this controller is the later, explicit orchestration
instruction.

This standing authorization does **NOT** supersede the controller's real hard
stop conditions. Still stop for genuine unresolved scientific/architecture
trade-offs, missing credentials, unexpected paid spend, protected TEST
authorization, broken provenance, failed regression gates, dangerous Git
state, or any other hard stop defined below.

Normal task boundary = continue automatically.
Real decision/safety boundary = stop for the user.

---

# Long-horizon state, context compaction, and usage-limit recovery

This is intentionally a multi-context, potentially multi-session job.

Claude Code may compact its context as it approaches the context-window
limit. Do not stop work merely because the remaining context/token budget is
low. Use the repository as durable state and continue after compaction.

Maintain this local, git-ignored controller state file:

```text
artifacts/phase3_controller/state.json
```

Create its parent directory if needed. This is runtime orchestration state,
not a tracked scientific result and must never be committed.
...
```json
{
  ...
  "status": "RUNNING",
  "current_task": "3.x",
  "last_completed_task": "3.y",
  "current_stage": "short machine-readable stage",
  "last_safe_commit": "<git sha or null>",
  "resume_instruction": "short description of the first incomplete action",
...
1. Set/update `status=RUNNING` at controller startup, before beginning each
   numbered task, after every meaningful checkpoint, and immediately after
   every successfully completed task before moving to the next one.
2. Before a long/expensive operation, record exactly what is about to run and
   the artifact/checkpoint identity needed to resume it.
3. After a long/expensive operation, update the state before doing unrelated
   work.
4. If a genuine hard stop needs the user, set `WAITING_USER` and record the
   exact decision required before returning control.
5. If an unrecoverable technical/integrity problem occurs, set `BLOCKED` and
   record the exact blocker.
6. Only after the Phase 3 exit audit succeeds may this become
   `PHASE3_COMPLETE`.
7. A provider/account usage limit is **not** a user decision. If Claude Code
   can no longer make model calls because the account usage allowance is
   exhausted, leave the state as `RUNNING` with a precise resume instruction.
   An external supervisor may restart/resume the session after the allowance
   refreshes.
8. On every fresh or resumed invocation, inspect this state file, Git,
   `Progress.md`, `PROJECT_EXECUTION.md`, result artifacts, and checkpoints.
   Resume the first incomplete action. Never restart completed expensive work
   merely because the model/session restarted.
9. Do not depend on conversation memory for correctness. Repository truth and
   durable artifacts win.
10. Never mark `WAITING_USER` merely because a context window is being
    compacted, a model response is ending, or a numbered task has completed.

If context is nearly exhausted, prioritize leaving the working tree and
runtime state resumable: finish or checkpoint the current atomic operation,
update the controller state, preserve valid experiment checkpoints, and avoid
starting an uncheckpointable destructive operation. Then continue after
compaction/resume.
...
The controller MUST automatically continue across a normal Phase 3 task
boundary when all of the conditions below are true. Do not downgrade this to
"MAY", do not ask for confirmation, and do not emit a terminal answer at that
boundary.

The controller automatically continues when all of these are true:
...
If they are all true, immediately execute the next Phase 3 task. A message
whose only purpose is asking the user to type `continue` is a controller
failure.
...
Do not print a terminal/final controller output at an ordinary successful
task boundary. Continue directly into the next roadmap task.

Only when the controller truly stops—for a genuine user decision, failure,
TEST authorization boundary, or successful Phase 3 exit—print:
